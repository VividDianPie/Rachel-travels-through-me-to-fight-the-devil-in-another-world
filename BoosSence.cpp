...
void BossSence::Init()
{
	...
	go->AddImage("³¡¾°»ðÕ½³¡", "bmp\\³¡¾°\\³¡¾°»ðÕ½³¡.bmp");
	go->AddBmp("³¡¾°»ðÕ½³¡", "³¡¾°»ðÕ½³¡", 0, 0, 2048, 768, 0xffffff);
	go->AddImage("±³¾°ÐÇ¿ÕÔÂÇò", "bmp\\³¡¾°\\±³¾°ÐÇ¿ÕÔÂÇò.bmp");
	go->AddBmp("±³¾°ÐÇ¿ÕÔÂÇò", "±³¾°ÐÇ¿ÕÔÂÇò", 0, 0, 2048, 1304);
...
void BossSence::Run()
{
	...
	go->DrawBmp("³¡¾°»ðÕ½³¡", 1024, 383 - go->GetCY() * 0.5, _Far_2);
	go->DrawBmp("±³¾°ÐÇ¿ÕÔÂÇò", 1024 + go->GetCX() - go->GetCX() * 0.5, 160 - go->GetCY() * 0.0005, _Far_1);
	switch (j)
	{
	...
	if (hero->GetPos().x<=16&& BOSSËÀ==boss->GetState()|| gi->Get(_GI_K_Y) == KS_DC)
	{
		kj->SetNextScene("»Ãº£É­ÁÖ");
		return;
	}
}
void BossSence::End()
